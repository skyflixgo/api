...
//Run movies
if ($type == 'movies') {
	//throttleRequest($type.'_'.$movieId);
...
function showBox_media($movieId, $title, $imdbId) {
    ...
    try {			
        ...
        if ($DEBUG) {
            echo 'Selected Video Link: ' . $selectedUrl . "</br></br>";
        }

        logDetails('showBox_media', 'extractFebBox', 'successful', $logTitle, $url, $selectedUrl, $type, $movieId, $type === 'series' ? $seriesCode : 'n/a');
...
function getDownloadUrl($share_key, $fid) {
    $url = 'https://www.febbox.com/file/file_download';
    $postData = array(
        'share_key' => $share_key,
        'fid' => $fid
    );
    ...
    if ($data['code'] === 1) {
        ...
        $thumb = $data['data']['thumb'] ?? '/static/index_img/file_type/' . $data['data']['file_icon'] . '.png';
        $fileName = $data['data']['file_name'];
        $fileSize = $data['data']['file_size'];

        // Return the download information in an array
        return [
            'thumb' => $thumb,
            'file_name' => $fileName,
            'file_size' => $fileSize,
